...
func demo(ctx context.Context, out io.Writer) error {
	...
	id, err := ids.Provision(ctx, "whatsapp", "+15551234567", "Alice")
	if err != nil {
		return fmt.Errorf("provision: %w", err)
	}
	fmt.Fprintf(out, "provisioned identity for whatsapp:+15551234567 → %s\n", id)
	...
	// continues on Slack.
	whatsapp, _ := ids.Resolve(ctx, "whatsapp", "+15551234567")
