...
**Follow-ups:** (a) SAML backend via `crewjam/saml` alongside OIDC — **deferred**: SAML is browser-flow with no HTTP surface on the daemon; assertion-in-chat-command is architecturally awkward. Revisit if the daemon grows an admin UI; ~~(b) directory-sync source (SCIM 2.0 pull or IdP-native API) so `ResolveTransportID` returns real answers~~ — **delivered in PR #132** (SCIM 2.0 Service Provider). `internal/auth/scim` ships full Users + Groups CRUD (GET / POST / PUT / DELETE), the one `?filter=userName eq "…"` operator every IdP integration uses, standard SCIM 2.0 error envelopes, and constant-time bearer-token auth. `internal/state/sqlite/scim.go` provides SQLite persistence with unique-constraint enforcement on userName / displayName / externalId, idempotent DELETE (SCIM 2.0 §3.6), a `scim_group_members` join table for O(1) membership lookup, and `LookupUserByExternalID` — the join point downstream SSO code will use to correlate a chat transport ID to an SCIM-provisioned user. `StartBackgroundServers` on `daemonWiring` spawns the SCIM HTTP server from every transport RunE with the same ctx lifetime. Three-condition licence gate matches SSO / RBAC / OPA / MultiParty pattern (no addr / no licence / no bearer_token → server skipped, doctor row surfaces which). Doctor rows `identity.sso.scim.{addr, bearer_token, licensed, directory}` (directory shows user + group counts); ~~(c) daemon-assembly wiring so transports actually call `Directory.VerifyToken`~~ — **delivered in PR #122**.
