...
**Follow-ups:** (a) SAML backend via `crewjam/saml` alongside OIDC — **deferred**: SAML is browser-flow with no HTTP surface on the daemon; assertion-in-chat-command is architecturally awkward. Revisit if the daemon grows an admin UI; ~~(b) directory-sync source (SCIM 2.0 pull or IdP-native API) so `ResolveTransportID` returns real answers~~ — **delivered in PR #132** + **PR #133** wires it end-to-end. PR #132 shipped the SCIM 2.0 Service Provider (`internal/auth/scim`, `internal/state/sqlite/scim.go`, HTTP handlers, three-condition licence gate). PR #133 closes the loop: `sso.OIDCDirectory` gains an optional `DirectoryStore` (attached via `WithStore`) that `ResolveTransportID` consults; `internal/state/sqlite/scim.go` gains `UserGroupNames(userID)` to hydrate group memberships from the join table; `internal/cli/sso_scim_adapter.go` adapts `*sqlitestore.SCIMStore` into `sso.DirectoryStore`, mapping SCIM User → sso.Identity (`Subject ← user.ID`, `Groups ← displayNames from scim_group_members`, `Email ← primary email fallback to first`, `DisplayName ← name.formatted fallback to userName`). Daemon `buildSSO` calls `WithStore` when SCIM is licensed + configured. RBAC / OPA / multi-party approvers now consult authoritative directory data instead of what a `/login` JWT happened to carry; ~~(c) daemon-assembly wiring so transports actually call `Directory.VerifyToken`~~ — **delivered in PR #122**.

**Estimate:** OIDC pilot 1d + daemon wiring 1d + SCIM 1d (originally 3–5d). SAML remains deferred pending admin-UI decision.
...
**Delivered — HA works end-to-end (PR #158, closing the sequence #116, #136–#140, #152, #154–#157):** `rousseau whatsapp` now assembles + runs under `state.driver=postgres`. Point two replicas at the same DSN and both see the same sessions, cron jobs, identities, SSO bindings, SCIM directory. `openStore` dispatches on driver + every extension constructor goes through a driver-agnostic factory (`openCronStore`, `openIdentityStore`, `openJIDMap`, `openClaudeSessionCache`, `openSessionCostStore`, `openAuditChainState`, `openSCIMStore`, `openSSOBindings`). The `openSQLiteStore` fail-CLOSED gate that predated this seam has been deleted — no code paths remain that hardcode the sqlite driver. `rousseau doctor` surfaces `state.driver` + a redacted `state.dsn`.

**Follow-ups:**
- ~~Port the extension tables (cron, jidmap, oauth, recall, session_cache, session_costs, identity, audit_chain_state, sso_bindings, scim) to Postgres~~ — **all delivered**: cron (#136), jidmap (#137), session_cache (#138), oauth (#139), session_costs (#140), recall/FTS via tsvector + `websearch_to_tsquery` + `ts_rank_cd` (#152), sso_bindings (#154), identity (#155), audit_chain_state (#156), scim triad with JSONB + FK cascades (#157). Every table mirrors the SQLite driver's method signatures and type-aliases the domain shape (CronJob, CostRecord, Summary, SearchHit, …) from the sqlite package, so the daemon can hold either concrete type via a shared interface without a driver-visible surface change downstream.
- ~~Daemon interface pass~~ — **delivered in PR #158**. `assembleDaemon` holds interfaces (`CronStoreI`, `SearchableStore`, `JIDMapI`, `ClaudeSessionCacheI`, `SessionCostStoreI`, `identity.Resolver`); `cron.Config.Store` widened to an in-package `Store` interface; `sqlite.CostRecorder.Store` widened to a `costWriter` interface. `buildSCIM` / `buildSSO` take `SearchableStore` instead of the concrete sqlite store.
...
- Redis session-cache adapter for read-hot session lookups. Not yet started; would sit in front of `state.Store` (either driver) as a read-through cache; only relevant once a specific workload proves the DB reads are hot enough to matter.

**Estimate:** shipped. Postgres pilot + Helm chart landed in ~1 day each; extension-table sweep + daemon interface pass took ~2 more days. Redis cache remains as a ~1-day follow-up gated on measured need.
